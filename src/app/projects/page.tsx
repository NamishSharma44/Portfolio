...
function CardWrapper({ href, children }: CardWrapperProps) {
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70">
      {children}
    </a>
  );
}
      ...
      <AnimatedSection className="section-card p-7 sm:p-8">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {projects.map((project, index) => (
            <CardWrapper key={project.title} href={project.link}>
              <AnimatedSection
                delay={0.08 * index}
